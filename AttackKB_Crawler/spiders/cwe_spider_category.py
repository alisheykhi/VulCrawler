...
from attackKB_crawler.items.cwe_mitre_items import CWEMitreCategoryItems
from attackKB_crawler.util.cwe_common_attribute import *


class CweMitreCategorySpider(XMLFeedSpider):
        ...
        'ITEM_PIPELINES': {
            ...
            #'attackKB_crawler.pipelines.cwe_mitre_pipelines.CWEMitreCategoryMySQLStorePipeline': 932,
            #'attackKB_crawler.pipelines.cwe_mitre_pipelines.CWECategoryJsonWriterPipeline': 900,
        },
        }
        def parse_node(self, response, node):
            self.logger.info("CWE Category Node")
            item = CWEMitreCategoryItems()
            item['callback'] = self.callback
            item['CWE_Type'] = "Category"
