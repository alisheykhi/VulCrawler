...
from attackKB_crawler.items.cwe_mitre_items import CWEMitreCompoundElementItems
from attackKB_crawler.util.cwe_common_attribute import *


class CweMitreCompoundElementSpider(XMLFeedSpider):

        name = 'CWE_CompoundElement'
        ...
        itertag = 'Compound_Element'
        iterator = 'xml'
        custom_settings     = {
        'ITEM_PIPELINES': {
            'attackKB_crawler.pipelines.post_pipeline.PostPipeline': 980,
            #'attackKB_crawler.pipelines.cwe_mitre_pipelines.CWEMitreCompoundElementMySQLStorePipeline': 932,
            #'attackKB_crawler.pipelines.cwe_mitre_pipelines.CWECompoundElementJsonWriterPipeline': 900,
        },
        }

        def __init__(self, callback, *args, **kwargs):
            super(CweMitreCompoundElementSpider, self).__init__(*args, **kwargs)
            self.callback = callback

        def parse_node(self, response, node):
            self.logger.info("CWE Compound_Element Node")
            item = CWEMitreCompoundElementItems()
            item['callback'] = self.callback
            item['CWE_Type'] = "Compound_Element"
